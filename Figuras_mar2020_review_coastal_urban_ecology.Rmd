...
grid.arrange(disc, aprox, anal, comp, subj,  ncol=1 )
...
legend_4 <- get_legend(comp) %>% as_ggplot()
legend_5 <- get_legend(subj) %>% as_ggplot()
...
grid.arrange(arrangeGrob(disc + theme(legend.position="none") + theme(axis.title.x = element_blank())+ xlab("Disciplinary focus"), 
                         ...
                         subj + theme(legend.position="none")+theme(axis.title.x = element_blank())+ xlab("Main research objective"),legend_1, legend_2, legend_3, legend_4, legend_5,layout_matrix = lay, left = text_grob("", rot = 90, vjust = 1, size =18)), bottom= text_grob("Proportion of articles", size =18)) 
